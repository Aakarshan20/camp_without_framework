...
$sql = "select aid,name,available,price from area order by aid asc";
$areas = getAll($conn,$sql);


if(!isset($_GET['category']) || ($_GET['category']=="all")){
	$sql  ="select * from ";
	$sql .= " (select id,year,month,day,orders.aid,booked,guest_name,name,phone,bank_account,status,orders.createtime,stayDays,price,table_type from orders ";
	$sql .= " left join area on area.aid=orders.aid ";
	$sql .= " where orders.aid in(select aid from area) and parent_id=0 ";
	
	$sql .=" union all ";
	$sql .= " select id,year,month,day,guestinfo.aid,booked,guest_name,name,phone,bank_account,status,guestinfo.createtime,stayDays,price,table_type from guestinfo ";
	$sql .= " left join area on area.aid=guestinfo.aid ";
	$sql .= " where guestinfo.aid in(select aid from area) and parent_id=0 ";
	
	$sql .=") as tmp order by year,month,day,aid asc";
	
	
}else if($_GET['category']=="guest"){
	$sql = "select id,year,month,day,guestinfo.aid,booked,guest_name,name,phone,bank_account,status,guestinfo.createtime,stayDays,price,table_type from guestinfo ";
	$sql .= " left join area on area.aid=guestinfo.aid ";
	$sql .= " where guestinfo.aid in(select aid from area) and parent_id=0 ";
	$sql .= " order by year,month,day,aid asc";
	// echo $sql;
